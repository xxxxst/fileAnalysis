...
import MainModel, { Size, ConfigMd } from '@/model/MainModel';
...
import HexView from '@/components/util/HexView/HexView.vue';
import IHexView from '@/components/util/HexView/HexViewTs';
import SimpleMonacoEditor from '@/components/util/SimpleMonacoEditor/SimpleMonacoEditor.vue';
import ISimpleMonacoEditor from '@/components/util/SimpleMonacoEditor/SimpleMonacoEditorTs';
import { TextMd } from '@/components/util/SimpleMonacoEditor/model/TextMd';
import { FileStructInfo, FileStruct, AddressMd, AddressAttrMd, StructAddressMd, FileStructAttr, StructAddressItem, StructAddressAttr } from '@/model/FileStruct';
import MonacoEditrCtl from '@/control/MonacoEditrCtl';
import MapPreview from '@/components/mapPreview/MapPreview.vue';
import HexViewFill from '@/components/hexViewFill/HexViewFill.vue';
import IHexViewFill from '@/components/hexViewFill/HexViewFillTs';
import StructFormatCtl from '@/control/StructFormatCtl';
import ComUtil from '@/util/ComUtil';
import MonacoHelpCtl from '@/control/MonacoHelpCtl';
import MainCtl from '@/control/MainCtl';

@Comp({ HexView, HexViewFill, SimpleMonacoEditor, MapPreview })
export default class Home extends Vue {
	@State() domSize: Size;
	@State() isDebug: boolean;
	@State() mapTypeLen: Record<string, number>;
	@State() cfgMd: ConfigMd;
	@State() setSaveDataInner: boolean;
	...
	get lstFileStruct() {
		return this.cfgMd.structData;
	}

	set lstFileStruct(val) {
		this.cfgMd.structData = val;
	}
	...
	@Watch(DEEP)
	cfgMdChanged() {
		if (this.setSaveDataInner) {
			return;
		}
		MainCtl.ins.cfgCtl.waitSaveConfig();
	}
	...
	// async calcCacheAddr(mapCacheAddr) {
	// 	for (var type in mapCacheAddr) {
	// 		for(var strAddr in mapCacheAddr[type]) {
	// 			var addr = parseInt(strAddr);
	// 			var val = await this.getFileValue(type, addr);
	// 			mapCacheAddr[type][addr] = val;
	// 		}
	// 	}
	// }
	...
	async updateAddress() {
		var map = this.monacoEditCtl.mapStruct;
		// var mapCacheAddr: any = {};
		// function saveAddr(type: string, addr: number) {
		// 	var obj = (mapCacheAddr[type] || (mapCacheAddr[type] = {}));
		// 	obj[addr] = 0;
		// }
		// function getAddrVal(type: string, addr: number) {
		// 	var obj = (mapCacheAddr[type] || (mapCacheAddr[type] = {}));
		// 	return obj[addr] || 0;
		// }
		// var objFun = {
		// 	fchar: function (addr) { saveAddr("char", addr); return 0; },
		// 	fbyte: function (addr) { saveAddr("byte", addr); return 0; },
		// 	fshort: function (addr) { saveAddr("short", addr); return 0; },
		// 	fushort: function (addr) { saveAddr("ushort", addr); return 0; },
		// 	fint: function (addr) { saveAddr("int", addr); return 0; },
		// 	fuint: function (addr) { saveAddr("uint", addr); return 0; },
		// 	fint64: function (addr) { saveAddr("int64", addr); return 0; },
		// 	fuint64: function (addr) { saveAddr("uint64", addr); return 0; },
		// };
		// var objFun2 = {
		// 	fchar: function (addr) { return getAddrVal("char", addr); },
		// 	fbyte: function (addr) { return getAddrVal("byte", addr); },
		// 	fshort: function (addr) { return getAddrVal("short", addr); },
		// 	fushort: function (addr) { return getAddrVal("ushort", addr); },
		// 	fint: function (addr) { return getAddrVal("int", addr); },
		// 	fuint: function (addr) { return getAddrVal("uint", addr); },
		// 	fint64: function (addr) { return getAddrVal("int64", addr); },
		// 	fuint64: function (addr) { return getAddrVal("uint64", addr); },
		// };

		// var strFun = "";
		// for (var key in objFun) {
		// 	strFun += `var ${key}=obj.${key};`;
		// }
		// var funStart = `(function(obj) { ${strFun} return (`;
		// var funEnd = `)})`;
		...
		// 	try {
		// 		mapEval[i] = eval(funStart + md.address + funEnd);
		// 		md.realAddr = mapEval[i](objFun);
		// 	} catch (ex) {
		// 		md.realAddr = -1;
		// 	}
		// }

		// await this.calcCacheAddr(mapCacheAddr);
		// console.info(mapCacheAddr);
		...
		for (var i = 0; i < this.arrAddress.length; ++i) {
			...
			try {
				md.realAddr = eval(funStart + md.address + funEnd)(objParam);
			} catch (ex) {
				md.realAddr = -1;
			}
			var arr = [];
			...
			var idxParam = mapCacheRst[md.name].length;
			var arrParam = (objParam[md.name] || (objParam[md.name] = []));
			var param = (arrParam[idxParam] || (arrParam[idxParam] = {}));

			param.$base = md.realAddr;
			param.$next = nextAddr;
			...
			var pos2 = md.realAddr;
			for (var j = 0; j < data.attrs.length; ++j) {
				var it2 = data.attrs[j];
				var len = this.getAttrLen(it2);
				var val = await this.getAttrValue(pos2, it2);

				param[it2.name] = val[0] || 0;

				var tmp2 = new StructAddressAttr();
				tmp2.address = pos2;
				tmp2.idx = j;
				tmp2.addrIdx = i;
				tmp2.values = val;
				md2.attrData.push(tmp2);
				pos2 += len;
			}
	...
	async getAttrValue(address: number, attr: FileStructAttr, maxLoadArrayCount = 10): Promise<any[]> {
		...
		for (var i = 0; i < count; ++i) {
			...
			switch (type) {
				...
				case "long":
				case "LONG": {
					// tmp = (((arr[pos + 3] << 24) >>> 0) + (arr[pos + 2] << 16) + (arr[pos + 1] << 8) + arr[pos]);
					if ((arr[pos + 3] & 0x80) != 0) {
						tmp = -(~tmp+1);
					}
					break;
				...
				case "DWORD": break;
				case "int64": {
					// tmp = ((arr[pos + 7] << 56) + (arr[pos + 6] << 48) + (arr[pos + 5] << 40) + (arr[pos + 4] << 32) + ((arr[pos + 3] << 24) >>> 0) + (arr[pos + 2] << 16) + (arr[pos + 1] << 8) + arr[pos]); break;
					if ((arr[pos + 7] & 0x80) != 0) {
						tmp = -(~tmp+1);
					}
					break;
				...
				case "uint64": break;
				case "float": {
					tmp = new Float32Array(new Uint8Array(arr.slice(pos, pos + 4)).buffer)[0];
					break;
				}
				case "double": {
					tmp = new Float64Array(new Uint8Array(arr.slice(pos, pos + 8)).buffer)[0];
					break;
				}
				default: break;
			}
	...
	// async getFileValue(type: string, address: number) {
	// 	if (!(type in this.mapTypeLen)) {
	// 		return 0;
	// 	}
